...
const renderContent = (content) => {
  ...
  html = html.replace(
    /`([^`]+)`/g,
    '<code class="bg-white/10 px-1.5 py-0.5 rounded text-rose-400 font-mono text-xs">$1</code>'
  );
...
<template>
  ...
  >
    <!-- Header -->
    <div
      class="p-4 border-b border-white/5 bg-slate-900/50 backdrop-blur-xl flex items-center justify-between z-10"
    >
      <div class="flex items-center gap-3">
        <div
          class="p-2 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-xl shadow-lg shadow-indigo-500/20"
        >
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            v-if="msg.role === 'assistant' && !msg.content && isTyping"
    ...
    <div class="p-6 bg-slate-900/50 backdrop-blur-xl border-t border-white/5">
      ...
      <div
        class="mt-3 flex items-center justify-center gap-4 text-[9px] text-slate-500 uppercase tracking-widest font-bold"
      >
        <div class="flex items-center gap-1.5">
          <Info class="w-3 h-3" /> 按回车发送
        </div>
        <div class="w-1 h-1 rounded-full bg-slate-800"></div>
        <div class="flex items-center gap-1.5">
          <Bot class="w-3 h-3" /> 云优自研内核
        </div>
      </div>
    </div>
  </div>
